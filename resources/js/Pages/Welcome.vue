...
const toggleCategory = (slug: string) => {
    const newCategory = props.filters?.category === slug ? null : slug;
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans">
                    ...
                    <div class="hidden md:flex flex-1 mx-10">
                        <div class="relative w-full max-w-lg mx-auto">
                            <input
                                ...
                                class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-full pl-5 pr-12 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow shadow-sm"
                                placeholder="Rechercher un produit..."
                            >
                            <button
                                @click="handleSearch"
                                class="absolute right-2 top-1 bottom-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full px-3 flex items-center justify-center transition"
                            >
        ...
        <div class="relative bg-white dark:bg-gray-800 overflow-hidden">
            ...
            <div class="lg:absolute lg:inset-y-0 lg:right-0 lg:w-1/2 bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
                <img class="h-56 w-full object-cover sm:h-72 md:h-96 lg:w-full lg:h-full opacity-90" src="https://images.unsplash.com/photo-1483985988355-763728e1935b?ixlib=rb-4.0.3&auto=format&fit=crop&w=1600&h=600&q=80" alt="Shopping Hero">
            </div>
        </div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    <div
                        ...
                        :class="[
                            'flex flex-col items-center p-4 rounded-xl shadow-sm hover:shadow-md hover:-translate-y-1 transition duration-300 cursor-pointer border',
                            filters?.category === cat.slug
                                ? 'bg-indigo-50 border-indigo-500 ring-2 ring-indigo-200 dark:bg-indigo-900'
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center mb-8">
                    <h2 class="text-3xl font-bold text-gray-900 dark:text-white">
                        <span v-if="filters?.category">Catégorie: {{ filters.category }}</span>
                        <span v-else-if="filters?.search">Recherche: "{{ filters.search }}"</span>
                        <span v-else>Nouveautés</span>
                    </h2>
                    <Link :href="route('home')" v-if="filters?.search || filters?.category" class="text-red-500 hover:text-red-700 text-sm font-medium">
                        (Effacer les filtres)
                    </Link>
                    <button v-else class="text-indigo-600 hover:text-indigo-800 font-medium">Tout voir &rarr;</button>
                </div>
